...
###开始查重,今天发所有的
sendtel<-c(as.character(frame_mike$V1),as.character(frame_add$`手机号`),as.character(frame_history_tel$...1))
...
##保存邮件序列
sendemail<-c(as.character(frame_mike$V2),as.character(frame_add$`邮箱`),as.character(frame_history_mail$...1))
##添加自己的联系方式,方便自己收到信息,以作测试
sendemail<-c(sendemail,'919086038@qq.com','1148235075@qq.com')
##去除其中的空格
sendemail<-str_replace_all(sendemail,' ','')
...
##今日电话发送列表存档
write.xlsx(list('今日之前所有的'=sendtel2,'今日之前所有的'=sendtel2),paste0(getwd(),'/MPB_event_message_',today,'_',as.character(addinfo$V1[2]),'_',length(sendtel2),'#.xlsx'),na='',row.names = FALSE,col.names = FALSE,sep=',')
##今日邮件发送列表存档
write.xlsx(list('今日之前所有的'=sendemail2,'今日之前所有的'=sendemail2),paste0(getwd(),'/MPB_event_mail_',today,'_',as.character(addinfo$V1[2]),'_',length(sendemail2),'#.xlsx'),na='',row.names = FALSE,col.names = FALSE,sep=',')
